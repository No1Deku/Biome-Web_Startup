...
async function init() {
    ...
    // 4. Handle missing ID - redirect to listings
    if (!listingId) {
        window.showToast('No property selected. Redirecting to listings...', 'warning');
        window.redirectTo('property-listings.html', 1500);
        return;
    }

    // 5. Handle invalid UUID
    if (!validateListingId(listingId)) {
        showErrorState(
            'Invalid Property',
            'This property link is invalid. Please return to the listings page.'
        );
        return;
    }
    ...
    try {
        ...
        // 7. Render progressively (immediate content)
        ...
        // 11. Load similar listings (non-blocking, after page render)
        ...
        if (error.message.includes('not found') || error.message.includes('access denied')) {
            showErrorState(
                ...
                'This property is no longer available. It may have been removed or is pending approval.'
            );
        } else {
            showErrorState(
